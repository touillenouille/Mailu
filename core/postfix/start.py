...
os.system("flock -n /home/mailu/data/queue/pid/master.pid rm /home/mailu/data/queue/pid/master.pid")
...
def start_podop():
    ...
    run_server(0, "postfix", "/tmp/podop2.socket", [
...
for postfix_file in glob.glob("/etc/sx/postfix/conf/*.cf"):
    conf.jinja(postfix_file, os.environ, os.path.join("/etc/postfix", os.path.basename(postfix_file)))

if os.path.exists("/home/mailu/data/overrides/postfix.cf"):
    for line in open("/home/mailu/data/overrides/postfix.cf").read().strip().split("\n"):
        if is_valid_postconf_line(line):
            os.system('postconf -e "{}"'.format(line))

if os.path.exists("/home/mailu/data/overrides/postfix.master"):
    for line in open("/home/mailu/data/overrides/postfix.master").read().strip().split("\n"):
        if is_valid_postconf_line(line):
            os.system('postconf -Me "{}"'.format(line))

for map_file in glob.glob("/home/mailu/data/overrides/*.map"):
    destination = os.path.join("/etc/postfix", os.path.basename(map_file))
    shutil.copyfile(map_file, destination)
    os.system("postmap {}".format(destination))
    os.remove(destination)

if os.path.exists("/home/mailu/data/overrides/mta-sts-daemon.yml"):
    shutil.copyfile("/home/mailu/data/overrides/mta-sts-daemon.yml", "/etc/mta-sts-daemon.yml")
else:
    conf.jinja("/etc/sx/postfix/conf/mta-sts-daemon.yml", os.environ, "/etc/mta-sts-daemon.yml")
...
if "RELAYUSER" in os.environ:
    ...
    conf.jinja("/etc/sx/postfix/conf/sasl_passwd", os.environ, path)
    os.system("postmap {}".format(path))

# Configure logrotate and start crond
if os.environ.get('POSTFIX_LOG_FILE'):
    conf.jinja("/etc/sx/postix/conf/logrotate.conf", os.environ, "/etc/logrotate.d/postfix.conf")
    os.system("/usr/sbin/crond")
    if os.path.exists("/home/mailu/data/overrides/logrotate.conf"):
        shutil.copyfile("/home/mailu/data/overrides/logrotate.conf", "/etc/logrotate.d/postfix.conf")
